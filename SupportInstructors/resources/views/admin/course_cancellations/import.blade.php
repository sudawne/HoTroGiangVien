...
@section('content')
    ...
    <div class="w-full px-4 py-4 h-[calc(100vh-80px)] flex flex-col">
        ...
        @if(!isset($previewData))
            ...
            <form action="{{ route('admin.course_cancellations.preview') }}" method="POST" enctype="multipart/form-data" class="flex flex-col h-full">
                @csrf
        @else
        ...
        <div class="bg-white dark:bg-[#1e1e2d] border border-slate-200 dark:border-slate-700 p-4 rounded-lg shadow-sm mb-4 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-20">
            ...
            <div class="flex items-center gap-4 flex-1">
                <a href="{{ route('admin.course_cancellations.index') }}" 
            ...
            <div class="flex items-center gap-3">
                ...
                @else
                    ...
                    <a href="{{ route('admin.course_cancellations.import') }}" class="px-4 py-2 text-slate-500 hover:text-slate-700 font-medium text-sm">
                        Chọn lại
                    </a>
                @endif
            </div>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach($previewData as $row)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ (!$row['student_exists'] || !$row['subject_exists']) ? 'bg-red-50/50 dark:bg-red-900/10' : '' }}" id="row-{{ $row['student_code'] }}">
                                    ...
                                    {{-- Cột MSSV --}}
                                    <td class="p-4 font-medium font-mono {{ !$row['student_exists'] ? 'text-red-600' : 'text-primary' }}">
                                        {{ $row['student_code'] }}
                                    </td>
                                    ...
                                    {{-- Cột Mã Môn --}}
                                    <td class="p-4 font-mono {{ !$row['subject_exists'] ? 'text-red-600 font-bold' : 'text-slate-500' }}">
                                        {{ $row['subject_code'] }}
                                    </td>
