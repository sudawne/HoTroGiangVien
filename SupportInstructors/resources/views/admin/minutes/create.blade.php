...
@section('content')
    ...
    <form action="{{ route($routePrefix . 'minutes.store') }}" method="POST"
                ...
                <div class="p-5 space-y-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Lớp sinh
                                    hoạt <span class="text-red-500">*</span></label>
                                <select name="class_id" onchange="window.location.href = '?class_id=' + this.value"
                                    ...
                                    @foreach ($classes as $cls)
                                        <option value="{{ $cls->id }}"
                                            {{ isset($currentClass) && $currentClass->id == $cls->id ? 'selected' : '' }}>
                                            {{ $cls->code }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="monitor_id" id="select-monitor" autocomplete="off">
                                    ...
                                    @foreach ($students as $st)
                                        <option value="{{ $st->id }}"
                                            {{ $currentClass->monitor_id == $st->id ? 'selected' : '' }}>
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between mb-3 text-xs">
                            ...
                            <div class="text-green-600">Có mặt: <strong
                                    id="display-present">{{ $students->count() }}</strong></div>
                        ...
                        <div>
                            <select name="absent_list[]" id="select-absent" multiple placeholder="Chọn người vắng..."
                                autocomplete="off">
                                @foreach ($students as $st)
                                    <option value="{{ $st->id }}">{{ $st->fullname }} ({{ $st->student_code }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex flex-wrap gap-2 mt-4">
                                <button type="button"
                                    onclick="insertToEditor('editor-content', 'Triển khai kế hoạch học tập học kỳ mới.')"
                                    class="px-3 py-1.5 bg-slate-100 text-slate-600 text-xs font-medium rounded-md hover:bg-slate-200 transition">+
                                    Kế hoạch học tập</button>
                                <button type="button"
                                    onclick="insertToEditor('editor-content', 'Nhắc nhở đóng học phí đúng hạn.')"
                                    class="px-3 py-1.5 bg-slate-100 text-slate-600 text-xs font-medium rounded-md hover:bg-slate-200 transition">+
                                    Học phí</button>
                            </div>
                        </div>
        ...
        <div
            ...
            <button type="submit" name="action" value="draft"
                class="px-5 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded shadow-sm transition">
                Lưu nháp
            </button>
            <button type="submit" name="action" value="publish"
                class="px-6 py-2 bg-primary hover:bg-primary/90 text-white font-bold rounded shadow-md shadow-primary/30 flex items-center gap-2 transition transform active:scale-95">
