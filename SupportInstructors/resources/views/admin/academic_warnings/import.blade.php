...
@section('content')
    ...
    <div class="w-full px-4 py-4 h-[calc(100vh-80px)] flex flex-col">
        ...
        @if (!isset($previewData))
            ...
            <form action="{{ route($routePrefix . 'academic_warnings.preview') }}" method="POST" enctype="multipart/form-data"
        ...
        <div
            class="bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-700 p-4 rounded-lg shadow-sm mb-4 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-20">

            {{-- Nhóm bên trái: Quay lại & Học kỳ --}}
            <div class="flex items-center gap-4 flex-1">
                <a href="{{ route($routePrefix . 'academic_warnings.index') }}"
        ...
        <div
            class="flex-1 bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden relative flex flex-col">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ !$row['exists'] ? 'bg-red-50/50 dark:bg-red-900/10' : '' }}"
                                    ...
                                    <td class="p-4">
                                        ...
                                        @elseif($row['warning_level'] == 2)
                                            <span class="badge-orange">Lần 2</span>
                                        @elseif($row['warning_level'] >= 3)
                                            <span class="badge-red">Thôi học</span>
                                        @else
                                            <span class="badge-gray">Khác</span>
                                        @endif
                                    </td>
    ...
    <style>
        .badge-yellow {
            @apply px-2 py-0.5 rounded text-xs font-semibold bg-yellow-100 text-yellow-700 border border-yellow-200;
        }
