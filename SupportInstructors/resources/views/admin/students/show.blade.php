...
@section('content')
    @php
        ...
        foreach ($allSemesterIds as $semId) {
            ...
            $tpRecord = $student->trainingPoints
                ? $student->trainingPoints->where('semester_id', $semId)->first()
                : null;
            ...
            elseif ($tpRecord) {
                $tpScore =
                    $tpRecord->final_score ?? // Điểm chốt
    ...
    <div class="w-full px-4 py-6" x-data="{
        ...
        <div class="mb-6">
                ...
                <div class="relative min-w-[250px]">
                    ...
                    <select x-model="selectedSemester"
                        ...
                        @foreach ($allSemesterIds->sortDesc() as $semId)
                            <option value="{{ $semId }}">
                                {{ $semestersInfo[$semId]->name ?? 'HK ???' }}
                                ({{ $semestersInfo[$semId]->academic_year ?? '' }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" x-show="selectedSemester && dataMap[selectedSemester]">
                ...
                <div
                    class="bg-white dark:bg-[#1e1e2d] border border-slate-200 dark:border-slate-700 p-5 rounded-lg shadow-sm flex items-center justify-between relative overflow-hidden group">
                    <div class="absolute right-0 top-0 h-full w-1 bg-blue-500"></div>
                    <div>
                        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wider">Điểm
                            TB (Hệ 4)</p>
                    ...
                    <div
                        class="w-14 h-14 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 flex items-center justify-center border border-blue-100 dark:border-blue-800 group-hover:scale-110 transition-transform">
                ...
                <div
                    ...
                    <div
                        class="w-14 h-14 rounded-full bg-purple-50 dark:bg-purple-900/20 text-purple-600 flex items-center justify-center border border-purple-100 dark:border-purple-800 group-hover:scale-110 transition-transform">
                ...
                <div
                    ...
                    <div>
                        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wider">Xếp
                            loại Học lực</p>
                        <div class="flex items-center">
                            <span class="text-sm font-bold px-3 py-1 rounded border uppercase shadow-sm"
                                x-text="dataMap[selectedSemester]?.rank" :class="dataMap[selectedSemester]?.rank_color">
                            </span>
                        </div>
                    </div>
                    <div
                        class="w-14 h-14 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 flex items-center justify-center border border-emerald-100 dark:border-emerald-800 group-hover:scale-110 transition-transform">
            ...
            <div x-show="!selectedSemester || !dataMap[selectedSemester]"
                class="p-6 bg-slate-50 border border-dashed border-slate-300 rounded-lg text-center text-slate-500 mt-4">
                                ...
                                <div
                                    ...
                                    <div class="w-full">
                                        <div class="flex justify-between items-start mb-2">
                                            <h4 class="font-bold text-base">Cảnh báo mức {{ $warning->warning_level }}
                                                <span
                                        ...
                                        @if ($warning->advisor_note)
                                            <div class="bg-white p-3 rounded-md text-sm border border-slate-200">
                                                <strong class="text-slate-600 flex items-center gap-1 mb-1"><span
                                                        class="material-symbols-outlined !text-[14px]">edit_note</span> Ghi
                                                    chú Cố vấn:</strong>
                                ...
                                <tbody
                                    ...
                                    @forelse($student->debts as $debt)
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <td class="px-4 py-3 text-center"><span
                                                    class="text-red-600 font-bold">{{ $debt->score ?? '--' }}</span></td>
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h4 class="font-bold text-slate-800 dark:text-slate-200 text-base">
                                                    {{ $result->semester->name ?? 'Học kỳ N/A' }}</h4>
                                                <p class="text-xs text-slate-500 font-mono">
                                                    {{ $result->semester->academic_year ?? '' }}</p>
                                            </div>
                                        </div>
                                        <div class="flex gap-2">
                                            <div
                                            ...
                                            <div>
                                                ...
                                                @php
                                                    ...
                                                    if (!$listTp && $student->trainingPoints) {
                                                        $tpRow = $student->trainingPoints
                                                            ->where('semester_id', $result->semester_id)
                                                            ->first();
                                                        if ($tpRow) {
                                                            $listTp =
                                                                $tpRow->final_score ?? ($tpRow->advisor_score ?? 0);
                                                        }
                                                    }
                                                @endphp
                                                <strong
                                                    class="text-slate-800 dark:text-slate-200">{{ $listTp ?? '--' }}</strong>
                                            </div>
                                            <div>
                                        ...
                                        <div class="text-sm text-slate-600 space-y-2">
                                            ...
                                            @if ($log->solution)
                                                <div
                                                    ...
                                                    <strong class="flex items-center gap-1 mb-1"><span
