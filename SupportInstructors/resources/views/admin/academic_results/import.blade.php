            ...
            @if (!isset($previewData))
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-10 border-4 border-dashed border-slate-100 dark:border-slate-800 m-4 rounded-xl bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group cursor-pointer"
                    ...
                    <div
                        ...
                        <span
                            class="material-symbols-outlined text-6xl text-indigo-300 dark:text-indigo-500 group-hover:text-indigo-600 transition-colors">analytics</span>
                    </div>
                    <h3 class="text-xl font-bold text-slate-700 dark:text-slate-200 mb-2">Nhập kết quả học tập</h3>
                    <p class="text-slate-500 dark:text-slate-400 mb-6 text-center">
                        Vui lòng chọn Học kỳ trước, sau đó tải file Excel lên.<br>
                        <span class="text-xs text-orange-500">Lưu ý: Dữ liệu bắt đầu từ dòng 5 (sau header).</span>
                    </p>
            ...
            @else
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach ($previewData as $row)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ $row['status'] == 'error' ? 'bg-red-50/50 dark:bg-red-900/10' : ($row['status'] == 'warning' ? 'bg-orange-50/30' : '') }}"
                                    id="row-{{ $row['mssv'] }}">
                                    <td
                                        class="p-4 font-medium font-mono {{ $row['status'] == 'error' ? 'text-red-600' : 'text-primary' }}">
                                        {{ $row['mssv'] }}</td>
                                    <td class="p-4 font-medium dark:text-slate-300">{{ $row['fullname'] }}</td>
                                    <td class="p-4 text-slate-500">{{ $row['class_code'] }}</td>

                                    {{-- Cột điểm --}}
                                    <td class="p-4 text-center text-slate-600">{{ $row['gpa_10'] }}</td>
                                    ...
                                    <td class="p-4 text-center">
                                        <span
                                            ...
                                            {{ $row['classification'] == 'Xuất sắc'
                                                ? 'bg-green-100 text-green-700 border-green-200'
                                                : ($row['classification'] == 'Giỏi'
                                                    ? 'bg-blue-100 text-blue-700 border-blue-200'
                                                    : ($row['classification'] == 'Khá'
                                    ...
                                    <td class="p-4 text-right">
                                        ...
                                        @elseif($row['status'] == 'warning')
                                            <div
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-orange-50 dark:bg-orange-900/20 border border-orange-100">
                                                <span
                                                    class="material-symbols-outlined text-[16px] text-orange-600">warning</span>
                                                <span class="text-xs font-bold text-orange-700">Khác lớp</span>
                                            </div>
                                        @else
                                            {{-- Nút Thêm Nhanh --}}
                                            <button type="button"
                                                onclick="openQuickAddModal('{{ $row['mssv'] }}', '{{ $row['fullname'] }}', '', '{{ $row['class_code'] ?? '' }}')"
                ...
                <div
                    ...
                    <div class="text-sm text-slate-500">
                        Đang xem trước <span
                            class="font-bold text-slate-900 dark:text-white">{{ count($previewData) }}</span> dòng.
                        <span class="text-xs ml-2 text-orange-500 italic">* Các dòng lỗi sẽ bị bỏ qua khi lưu.</span>
                    </div>
                    <button type="submit"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" id="qa_mssv" name="mssv" readonly
                                        class="w-full bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded-sm text-sm focus:ring-primary text-slate-500">
                                </div>
                                <div>
                                    <label
                                    ...
                                    <input type="date" id="qa_dob" name="dob"
                                        class="w-full bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded-sm text-sm focus:ring-primary">
                                </div>
                            </div>
                            <div>
