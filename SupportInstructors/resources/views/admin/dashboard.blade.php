...
@section('content')
    <div class="max-w-[1400px] mx-auto flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <a href="{{ route($routePrefix . 'students.index') }}" class="group block">
                <div
                    class="bg-white dark:bg-[#1e1e2d] border border-slate-200 dark:border-slate-700 rounded p-4 flex flex-col justify-between h-28 shadow-sm group-hover:shadow-md group-hover:-translate-y-1 transition-all">
                    <div class="flex justify-between items-start">
                        <p class="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase tracking-wide">Tổng số
                            sinh viên</p>
                    ...
                    <div class="flex items-end gap-2">
                        <h2 class="text-3xl font-bold text-slate-800 dark:text-white">{{ number_format($totalStudents) }}
                        </h2>
            ...
            <a href="{{ route($routePrefix . 'academic_warnings.index') }}" class="group block">
                <div
                    class="bg-white dark:bg-[#1e1e2d] border border-red-100 dark:border-red-900/30 rounded p-4 flex flex-col justify-between h-28 shadow-sm group-hover:shadow-md group-hover:-translate-y-1 transition-all relative overflow-hidden">
                    <div class="absolute right-0 top-0 w-16 h-16 bg-red-50 dark:bg-red-900/10 rounded-bl-full -mr-4 -mt-4">
                    </div>
                    <div class="flex justify-between items-start relative z-10">
                        <p class="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase tracking-wide">Cảnh báo
                            học tập</p>
            ...
            <a href="{{ route($routePrefix . 'course_cancellations.index') }}" class="group block">
                <div
                    class="bg-white dark:bg-[#1e1e2d] border border-slate-200 dark:border-slate-700 rounded p-4 flex flex-col justify-between h-28 shadow-sm group-hover:shadow-md group-hover:-translate-y-1 transition-all">
                    <div class="flex justify-between items-start">
                        <p class="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase tracking-wide">Xóa học
                            phần</p>
            ...
            <a href="{{ route($routePrefix . 'minutes.index') }}" class="group block">
                <div
                    class="bg-white dark:bg-[#1e1e2d] border border-slate-200 dark:border-slate-700 rounded p-4 flex flex-col justify-between h-28 shadow-sm group-hover:shadow-md group-hover:-translate-y-1 transition-all">
                    <div class="flex justify-between items-start">
                        <p class="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase tracking-wide">Biên bản
                            họp lớp</p>
        ...
        <div
            ...
            <form action="{{ route($routePrefix . 'dashboard') }}" method="GET"
                class="flex items-center gap-2 w-full md:w-auto">
                <span class="text-sm font-medium text-slate-600 dark:text-slate-300 ml-1 hidden sm:inline">Dữ liệu hiển
                    thị:</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div
                ...
                <div
                    ...
                    <h3 class="font-bold text-slate-800 dark:text-white truncate">Top Sinh viên bị cảnh báo
                        ({{ $semesterLabel }})</h3>
                    <a class="text-xs font-medium text-primary hover:underline"
                        href="{{ route($routePrefix . 'academic_warnings.index') }}">Xem tất cả</a>
                </div>
                        ...
                        <tbody class="text-sm divide-y divide-slate-100 dark:divide-slate-700">
                            @forelse($studentsToWatch as $student)
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                    <td class="px-4 py-2.5 font-mono text-slate-600 dark:text-slate-300 text-xs">
                                    ...
                                    <td class="px-4 py-2.5 text-slate-500 text-xs">
                                        {{ $student->studentClass->code ?? 'N/A' }}</td>
                                    <td class="px-4 py-2.5 text-center font-bold text-slate-700 dark:text-slate-300">
                                        {{ $warning->gpa_term ?? '0.0' }}</td>
                                    <td class="px-4 py-2.5 text-center">
                                        @if ($level == 1)
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-yellow-50 text-yellow-700 border border-yellow-100">Mức
                                                1</span>
                                        @elseif($level == 2)
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-orange-50 text-orange-700 border border-orange-100">Mức
                                                2</span>
                                        @elseif($level >= 3)
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-50 text-red-700 border border-red-100">Buộc
                                                thôi học</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-2.5 text-right">
                                        <a href="{{ route($routePrefix . 'students.show', $student->id) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-slate-400">
                                        <span
                                            class="material-symbols-outlined !text-[32px] block mb-2 opacity-50">check_circle</span>
                ...
                <div
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a class="text-[10px] text-slate-400 hover:text-primary uppercase font-bold tracking-wider"
                            href="{{ route($routePrefix . 'notifications.index') }}">Xem hết</a>
                    </div>
                    <div class="flex flex-col gap-4">
                        @forelse($recentNotifications as $notify)
                            <a href="{{ route($routePrefix . 'notifications.show', $notify->id) }}"
                                class="flex gap-3 items-start pb-3 border-b border-slate-100 dark:border-slate-800 last:border-0 last:pb-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors -mx-2 px-2 rounded">
                                <div
                                    class="mt-1 min-w-1.5 h-1.5 rounded-full {{ $notify->type == 'urgent' ? 'bg-red-500' : 'bg-primary' }}">
                                </div>
                                <div>
                                    <p
                                        class="text-sm font-medium text-slate-800 dark:text-slate-200 leading-snug line-clamp-2">
                                        {{ $notify->title }}</p>
                                    <p class="text-xs text-slate-400 mt-1">{{ $notify->created_at->format('d/m/Y') }} • Hệ
                                        thống</p>
                                </div>
                            </a>
                        @empty
                ...
                <div
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route($routePrefix . 'minutes.create') }}"
                            class="p-1 hover:bg-slate-100 rounded text-slate-400"><span
                                class="material-symbols-outlined !text-[15px]">add</span></a>
                    </div>
                    <div class="flex flex-col gap-3">
                        @forelse($upcomingMeetings as $meeting)
                            @php $meetingDate = \Carbon\Carbon::parse($meeting->created_at); @endphp
                            <a href="{{ route($routePrefix . 'minutes.show', $meeting->id) }}"
                                class="flex gap-3 bg-slate-50 dark:bg-slate-800/50 p-2 rounded border border-slate-100 dark:border-slate-800 hover:border-primary/30 transition-colors">
                                <div
                                    ...
                                    <span
                                        class="text-[10px] font-bold text-red-500 uppercase">{{ $meetingDate->format('M') }}</span>
                                    <span
                                        class="text-lg font-bold text-slate-800 dark:text-white leading-none">{{ $meetingDate->format('d') }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span
                                            class="text-[10px] bg-slate-200 dark:bg-slate-600 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300">{{ $meetingDate->format('H:i') }}</span>
                                        <span
                                            class="text-xs text-slate-500 truncate">{{ $meeting->location ?? 'Online' }}</span>
                                    </div>
                                </div>
                            </a>
                        @empty
