...
@section('content')
    ...
    <div class="w-full px-4 py-4 h-[calc(100vh-80px)] flex flex-col">
        ...
        @else
            ...
            <form action="{{ route('admin.academic_results.store_import') }}" method="POST" class="flex flex-col h-full">
                @csrf
        ...
        <div class="bg-white dark:bg-[#1e1e2d] border border-slate-200 dark:border-slate-700 p-4 rounded-lg shadow-sm mb-4 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-20">
            ...
            <div class="flex items-center gap-3">
                ...
                @else
                    ...
                    <a href="{{ route('admin.academic_results.import') }}" class="px-4 py-2 text-slate-500 hover:text-slate-700 font-medium text-sm">
                        Chọn lại
                    </a>
                @endif
            </div>
            ...
            @if(!isset($previewData))
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-10 border-4 border-dashed border-slate-100 dark:border-slate-800 m-4 rounded-xl bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group cursor-pointer" onclick="document.getElementById('file-upload').click()">
                    <div class="bg-white dark:bg-slate-700 p-6 rounded-full shadow-sm mb-4 group-hover:scale-110 transition-transform duration-300">
                        <span class="material-symbols-outlined text-6xl text-indigo-300 dark:text-indigo-500 group-hover:text-indigo-600 transition-colors">analytics</span>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead class="bg-slate-50 dark:bg-slate-800 sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th class="p-4 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700 text-center">GPA (10)</th>
                                <th class="p-4 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700 text-center">GPA (4)</th>
                                <th class="p-4 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700 text-center">Xếp loại</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach($previewData as $row)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ ($row['status'] == 'error') ? 'bg-red-50/50 dark:bg-red-900/10' : (($row['status'] == 'warning') ? 'bg-orange-50/30' : '') }}" id="row-{{ $row['mssv'] }}">
                                    ...
                                    <td class="p-4 text-center font-bold text-slate-800 dark:text-slate-200">{{ $row['gpa_4'] }}</td>
                        ...
                        <div class="px-6 py-4 space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" id="qa_dob" name="dob" class="w-full bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded-sm text-sm focus:ring-primary">
                                </div>
                            </div>
                            <div>
