...
@section('content')
    ...
    <form action="{{ route($routePrefix . 'minutes.update', $minute->id) }}" method="POST"
        class="h-[calc(100vh-65px)] flex flex-col overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div
            ...
            <div class="text-xs text-slate-400 italic">
                @if ($minute->status == 'published')
                    <span class="text-emerald-600 font-bold flex items-center gap-1"><span
                            class="material-symbols-outlined text-[16px]">lock</span> Đã chốt sổ</span>
                @else
                    Đang chỉnh sửa...
                @endif
            </div>
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <div
                class="w-[400px] bg-slate-50 dark:bg-[#151521] border-r border-slate-200 dark:border-slate-700 flex flex-col overflow-y-auto custom-scrollbar">
                <div class="p-5 space-y-6">
                    {{-- Group 1: Thông tin chung --}}
                    <div
                        class="bg-white dark:bg-[#1e1e2d] p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Lớp sinh
                                    hoạt</label>
                                <input type="text" readonly
                                    value="{{ $minute->studentClass->code }} - {{ $minute->studentClass->name }}"
                                    class="w-full rounded border-slate-200 bg-slate-100 text-sm text-slate-500 font-bold cursor-not-allowed">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Tiêu đề
                                    biên bản <span class="text-red-500">*</span></label>
                                <input type="text" name="title" required
                                    class="w-full rounded border-slate-300 text-sm focus:ring-primary font-bold"
                                    value="{{ old('title', $minute->title) }}">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Học kỳ
                                    <span class="text-red-500">*</span></label>
                                <select name="semester_id"
                                    class="w-full rounded border-slate-300 text-sm focus:ring-primary">
                                    @foreach ($semesters as $sem)
                                        <option value="{{ $sem->id }}"
                                            {{ $minute->semester_id == $sem->id ? 'selected' : '' }}>
                    ...
                    <div
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Kết
                                        thúc</label>
                    ...
                    <div
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" readonly
                                    value="{{ $minute->studentClass->advisor->user->name ?? 'Chưa cập nhật' }}"
                            ...
                            <div>
                                ...
                                <select name="monitor_id" id="select-monitor" autocomplete="off">
                                    ...
                                    @foreach ($students as $st)
                                        <option value="{{ $st->id }}"
                                            {{ $minute->monitor_id == $st->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="secretary_id" id="select-secretary" autocomplete="off">
                                    ...
                                    @foreach ($students as $st)
                                        <option value="{{ $st->id }}"
                                            {{ $minute->secretary_id == $st->id ? 'selected' : '' }}>
                                            {{ $st->fullname }} ({{ $st->student_code }})
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div
                        ...
                        <div>
                            <select name="absent_list[]" id="select-absent" multiple placeholder="Chọn người vắng..."
                                autocomplete="off">
                                @foreach ($students as $st)
                                    @php
            ...
            <div class="flex-1 bg-slate-100 dark:bg-slate-900 overflow-y-auto custom-scrollbar p-8">
                <div class="max-w-4xl mx-auto space-y-8 pb-20">
                    ...
                    {{-- MỤC II --}}
                    <div
                        class="bg-white dark:bg-[#1e1e2d] rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <div
                            class="bg-slate-50 dark:bg-slate-800 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                            <h2 class="text-lg font-bold text-slate-800 dark:text-white uppercase">II. Nội dung cuộc họp
                            </h2>
                        </div>
                        <div class="p-6">
                            ...
                            <div class="flex flex-wrap gap-2 mt-4">
                                <button type="button"
                                    onclick="insertToEditor('editor-discussions', 'Triển khai kế hoạch học tập học kỳ mới.')"
                                    class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-medium rounded-md transition border border-slate-200">+
                                    Kế hoạch học tập</button>
                                <button type="button"
                                    onclick="insertToEditor('editor-discussions', 'Nhắc nhở đóng học phí đúng hạn.')"
                                    class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-medium rounded-md transition border border-slate-200">+
                                    Học phí</button>
                                <button type="button"
                                    onclick="insertToEditor('editor-discussions', 'Đánh giá điểm rèn luyện.')"
                                    class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-medium rounded-md transition border border-slate-200">+
                                    Điểm rèn luyện</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="bg-white dark:bg-[#1e1e2d] rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <div
                            class="bg-slate-50 dark:bg-slate-800 px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <div
                        ...
                        <div
                            class="bg-slate-50 dark:bg-slate-800 px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                            <h2 class="text-lg font-bold text-slate-800 dark:text-white uppercase">IV. Kiến nghị của sinh
                                viên</h2>
                        </div>
                        <div class="p-6">
        ...
        <div
            class="h-16 bg-white dark:bg-[#1e1e2d] border-t border-slate-200 dark:border-slate-700 flex items-center justify-end px-6 gap-3 z-20 shrink-0">

            <a href="{{ route($routePrefix . 'minutes.index') }}"
                class="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded font-medium transition">
                Hủy bỏ
            </a>

            {{-- KHU VỰC DUYỆT CỦA ADMIN --}}
            @if ((Auth::user()->role_id ?? 0) == 1)
                <button type="submit" formaction="{{ route($routePrefix . 'minutes.approve', $minute->id) }}"
            ...
            <button type="submit" name="action" value="draft"
                class="px-5 py-2 bg-primary hover:bg-primary/90 text-white font-bold rounded shadow-md shadow-primary/30 flex items-center gap-2 transition transform active:scale-95">
