...
@section('content')
    <div class="flex flex-col h-[calc(100vh-100px)]">
            ...
            <form action="{{ route($routePrefix . 'academic_warnings.store') }}" method="POST">
                ...
                <div class="flex gap-2">
                    <a href="{{ route($routePrefix . 'academic_warnings.import') }}"
                        class="px-4 py-2 border border-slate-300 rounded-lg text-slate-600 hover:bg-slate-50 text-sm font-medium">Quay
                        lại</a>
                    <button type="submit"
                        class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-indigo-700 text-sm font-medium shadow-md">
                        Xác nhận Import
                    </button>
                </div>
            </form>
        ...
        <div
            class="flex-1 bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col">
                ...
                <table class="w-full text-left border-collapse text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($previewData as $index => $row)
                            <tr class="transition-colors hover:bg-slate-50 {{ !$row['exists'] ? 'bg-red-50 dark:bg-red-900/10' : '' }}"
                                id="row-{{ $row['mssv'] }}">
                                <td class="p-3 font-medium {{ !$row['exists'] ? 'text-red-600' : 'text-primary' }}">
                                    {{ $row['mssv'] }}</td>
                                ...
                                <td
                                    class="p-3 text-center {{ $row['credits_failed'] > 0 ? 'text-red-500 font-bold' : '' }}">
                                    {{ $row['credits_failed'] }}</td>
                                <td class="p-3 text-xs text-slate-500 max-w-[200px] truncate" title="{{ $row['reason'] }}">
                                    {{ $row['reason'] }}</td>
                                <td class="p-3">
                                    ...
                                    @else
                                        <span class="px-2 py-0.5 rounded text-xs bg-slate-100 text-slate-600">Theo
                                            dõi</span>
                                    @endif
                                </td>
                                <td class="p-3 text-right">
                                    ...
                                    @else
                                        <button
                                            onclick="quickAddStudent('{{ $row['mssv'] }}', '{{ $row['fullname'] }}', '{{ $row['dob'] }}')"
