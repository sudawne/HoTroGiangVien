...
<div class="w-full px-4 py-6" x-data="{ showCreateModal: false, showEditModal: false }">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button type="button" 
                                        ...
                                        class="p-1.5 bg-blue-50 text-blue-600 rounded hover:bg-blue-100 transition-colors" title="Sửa">
                                    ...
                                    <form action="{{ route('admin.subjects.destroy', $subject->id) }}" method="POST" 
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-1.5 bg-red-50 text-red-600 rounded hover:bg-red-100 transition-colors" title="Xóa">
    ...
    <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" x-cloak style="display: none;">
        <div class="bg-white dark:bg-[#1e1e2d] w-full max-w-md rounded-lg shadow-xl overflow-hidden" @click.away="showCreateModal = false">
            ...
            <form action="{{ route('admin.subjects.store') }}" method="POST" class="p-6 space-y-4">
                @csrf
                <div>
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" x-cloak style="display: none;">
        <div class="bg-white dark:bg-[#1e1e2d] w-full max-w-md rounded-lg shadow-xl overflow-hidden" @click.away="showEditModal = false">
            ...
            <form id="editForm" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Mã môn học <span class="text-red-500">*</span></label>
                    <input type="text" name="code" id="edit_code" required class="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Tên môn học <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="edit_name" required class="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Số tín chỉ <span class="text-red-500">*</span></label>
                    <input type="number" name="credits" id="edit_credits" required min="0" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary">
                </div>
                <div class="flex justify-end pt-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary/90">Cập nhật</button>
                </div>
            </form>
        </div>
    </div>
