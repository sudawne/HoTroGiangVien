...
@section('content')
    <div class="flex flex-col h-[calc(100vh-100px)]">
            ...
            <form action="{{ route($routePrefix . 'course_cancellations.store_import') }}" method="POST">
                ...
                <div class="flex gap-2">
                    <a href="{{ route($routePrefix . 'course_cancellations.import') }}"
                        class="px-4 py-2 border border-slate-300 rounded-lg text-slate-600 hover:bg-slate-50 text-sm font-medium transition-colors">
                        Quay lại
                    </a>
                    <button type="submit"
                        class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-indigo-700 text-sm font-medium shadow-md transition-colors flex items-center gap-2">
        ...
        <div
            class="flex-1 bg-white dark:bg-[#1e1e2d] rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col">
            <div class="overflow-auto flex-1">
                <table class="w-full text-left border-collapse text-sm">
                    <thead class="bg-slate-50 dark:bg-slate-800 sticky top-0 z-10 shadow-sm">
                        <tr>
                            <th class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700">
                                Lớp</th>
                            <th class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700">
                                MSSV</th>
                            <th class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700">
                            ...
                            <th class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700">
                                Tên Môn</th>
                            <th
                                class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700 text-center">
                                TC</th>
                            <th
                                class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700 text-center">
                                Lý do</th>
                            <th
                                class="p-3 font-semibold text-slate-600 dark:text-slate-400 border-b dark:border-slate-700 text-right">
                                Trạng thái</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @foreach ($previewData as $row)
                            ...
                            <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 {{ !$row['student_exists'] || !$row['subject_exists'] ? 'bg-red-50 dark:bg-red-900/10' : '' }}"
                                ...
                                {{-- MSSV --}}
                                <td
                                    class="p-3 font-medium font-mono {{ !$row['student_exists'] ? 'text-red-600 font-bold' : 'text-primary' }}">
                                    {{ $row['student_code'] }}
                                </td>
                                ...
                                {{-- Tên Môn --}}
                                <td class="p-3 text-slate-600 dark:text-slate-400 max-w-[200px] truncate"
                                ...
                                <td class="p-3 text-center">
                                    <span
                                        class="px-2 py-0.5 rounded text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600">
                                        {{ $row['reason'] }}
                                    </span>
                                </td>
                                ...
                                <td class="p-3 text-right action-cell">
                                    @if ($row['student_exists'] && $row['subject_exists'])
                                        <span
                                            class="inline-flex items-center justify-end gap-1 text-green-600 text-xs font-bold">
                                    ...
                                    @else
                                        <div class="flex flex-col items-end gap-1">
                                            ...
                                            @if (!$row['student_exists'])
                                                <span
                                                    class="px-2 py-0.5 rounded text-xs bg-red-100 text-red-700 border border-red-200 font-bold">
                                                    Chưa có SV
                                                </span>
                                            @endif
                                            ...
                                            @if (!$row['subject_exists'])
                                                <button type="button"
                                                    ...
                                                    <span
                                                        class="material-symbols-outlined text-sm group-hover:scale-110 transition-transform">library_add</span>
                                                    Thêm Môn
                                                </button>
                                            @endif
                                        </div>
            ...
            {{-- FOOTER INFO --}}
            <div
                class="p-3 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-xs text-slate-500">
                <span>Hiển thị <span class="font-bold text-slate-800 dark:text-white">{{ count($previewData) }}</span> dòng
                    dữ liệu.</span>
    ...
    <div id="quickAddSubjectModal" class="fixed inset-0 z-[100] hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
                ...
                <div
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="mt-3 text-center sm:ml-0 sm:mt-0 sm:text-left">
                                ...
                                <p class="text-sm text-slate-500 mt-1">Môn này chưa có trong CSDL. Thêm ngay để import hợp
                                    lệ.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form id="quickAddSubjectForm">
                        <div class="px-6 py-4 space-y-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Mã
                                    Học Phần</label>
                                <input type="text" id="qas_code" name="code" readonly
                                    class="w-full bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded-sm text-sm font-mono text-slate-500 cursor-not-allowed font-bold">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Tên
                                    Môn Học <span class="text-red-500">*</span></label>
                                <input type="text" id="qas_name" name="name"
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Số
                                    Tín Chỉ <span class="text-red-500">*</span></label>
                                <input type="number" id="qas_credits" name="credits"
                                    class="w-full bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded-sm text-sm focus:ring-primary focus:border-primary"
                                    required min="0">
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <button type="submit" id="btn-qas-save"
                                class="inline-flex w-full justify-center rounded-sm bg-orange-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-orange-500 sm:ml-3 sm:w-auto transition-all">
                                <span class="material-symbols-outlined !text-[18px] mr-1">save</span> Lưu Môn Học
                            </button>
                            <button type="button" onclick="closeQuickAddSubjectModal()"
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        const codeCells = document.querySelectorAll(
                            `.subject-code-cell[data-code="${targetCode}"]`);
                        ...
                        codeCells.forEach(cell => {
                            ...
                            const hasStudentError = row.querySelector(
                            '.bg-red-100'); // Class của badge lỗi SV
