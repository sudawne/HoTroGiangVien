...
@forelse($trainingPoints as $index => $point)
    ...
    <tr
        class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-50 dark:border-slate-800">
        ...
        <td class="px-6 py-4">
            <div class="flex items-center gap-3">
                {{-- Avatar chữ cái --}}
                <div
                    class="w-8 h-8 rounded-sm bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-500 border border-slate-200 dark:border-slate-600">
                    {{ substr($point->student->fullname ?? 'U', 0, 1) }}
                </div>
                <div>
                    <div class="font-bold text-slate-800 dark:text-white text-sm">
                        {{ $point->student->fullname ?? 'N/A' }}</div>
                    <div class="text-xs text-slate-500 font-mono mt-0.5">{{ $point->student->student_code ?? '---' }}
                    </div>
                </div>
            </div>
        </td>
        ...
        <td class="px-6 py-4 text-center">
            <span
                class="inline-flex items-center justify-center px-2.5 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wide border {{ $rankClass }}">
                {{ $rankName }}
            </span>
        </td>
        ...
        <td class="px-6 py-4 text-right">
            <a href="{{ route($routePrefix . 'students.show', $point->student_id) }}"
                class="text-slate-400 hover:text-primary transition-colors p-2 rounded-sm hover:bg-slate-100 inline-block"
                title="Chỉnh sửa">
