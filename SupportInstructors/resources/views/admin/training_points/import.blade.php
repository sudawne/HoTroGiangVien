...
@section('content')
    ...
    <div class="w-full h-[calc(100vh-80px)] flex flex-col">
        ...
        @if (!isset($previewData))
            ...
            <form action="{{ route($routePrefix . 'training_points.preview') }}" method="POST" enctype="multipart/form-data"
                class="flex flex-col h-full">
                @csrf
            @else
                {{-- FORM GIAI ĐOẠN 2: CONFIRM STORE --}}
                <form action="{{ route($routePrefix . 'training_points.store_import') }}" method="POST"
                    class="flex flex-col h-full">
                    @csrf
        ...
        <div class="rounded-lg mb-4 flex flex-col xl:flex-row xl:items-center justify-between gap-4 sticky top-0 z-20">
            ...
            <x-page-header title="Nhập dữ liệu" description="Tải lên file Excel để cập nhật điểm rèn luyện"
                :routePrefix="$routePrefix" :breadcrumbs="[
                    ['label' => 'Điểm rèn luyện', 'url' => route($routePrefix . 'training_points.index')],
                    ['label' => 'Nhập dữ liệu'],
                ]" />
            ...
            @if (!isset($previewData))
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-10 border-4 border-dashed border-slate-100 dark:border-slate-800 m-4 rounded-xl bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group cursor-pointer"
                    ...
                    <div
                        ...
                        <span
                            class="material-symbols-outlined text-6xl text-emerald-300 dark:text-emerald-500 group-hover:text-emerald-600 transition-colors">military_tech</span>
                    </div>
                            ...
                            @foreach ($previewData as $row)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ $row['status'] == 'error' ? 'bg-red-50/50 dark:bg-red-900/10' : ($row['status'] == 'warning' ? 'bg-orange-50/30' : '') }}"
                                    ...
                                    <td class="p-4 text-center font-bold text-slate-700 dark:text-slate-300">
                                        {{ $row['class_score'] }}</td> {{-- Mặc định lấy điểm lớp làm điểm khoa --}}
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label
                                        ...
                                        sinh</label>
                                    <input type="text" id="qa_dob" name="dob"
