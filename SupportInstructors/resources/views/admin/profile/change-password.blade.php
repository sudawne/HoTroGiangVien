...
@section('content')
    <div class="max-w-md mx-auto mt-10 bg-white dark:bg-[#1e1e2d] p-6 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700"
        ...
        <form action="{{ route($routePrefix . 'profile.update_password') }}" method="POST">
            @csrf
            <div class="mb-6">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Email giảng viên</label>
                <div class="flex gap-2">
                    <input type="text" value="{{ auth()->user()->email }}" disabled
                        class="flex-1 bg-slate-100 dark:bg-slate-800 border-slate-300 rounded-lg text-sm">
                    <button type="button"
                        @click="
                    loading = true;
                    fetch('{{ route($routePrefix . 'profile.send_otp') }}', {method:'POST', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'}})
                    .then(() => { otpSent = true; loading = false; timer = 60; let interval = setInterval(() => { timer--; if(timer <= 0) clearInterval(interval) }, 1000); });
                "
                        :disabled="loading || timer > 0"
                        class="px-4 py-2 bg-primary text-white rounded-lg text-sm font-bold disabled:opacity-50">
            ...
            <div x-show="otpSent" x-transition>
                ...
                <button type="submit"
                    class="w-full py-3 bg-primary text-white rounded-lg font-bold shadow-lg shadow-primary/30">Xác nhận đổi
                    mật khẩu</button>
            </div>
        </form>
    </div>
