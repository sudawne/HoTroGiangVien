    ...
    <div class="max-w-[1400px] mx-auto flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                            ...
                            @foreach ($academicYears as $year => $semesters)
                                <div x-data="{ open: {{ request('academic_year') == $year ? 'true' : 'false' }} }">
                                    ...
                                    <div x-show="open" x-collapse
                                        class="pl-4 mt-1 space-y-1 border-l-2 border-slate-100 ml-2">
                                        {{-- Link lọc theo cả năm --}}
                                        <a href="{{ route($routePrefix . 'minutes.index', ['academic_year' => $year]) }}"
                                        ...
                                        @foreach ($semesters as $sem)
                                            <a href="{{ route($routePrefix . 'minutes.index', ['semester_id' => $sem->id]) }}"
            ...
            <div class="lg:col-span-3 flex flex-col gap-4">
                ...
                @if ($minutes->isEmpty())
                    <div
                        class="bg-white dark:bg-[#1e1e2d] p-10 rounded-xl border border-dashed border-slate-300 text-center">
                    ...
                    @foreach ($minutes as $minute)
                        <div
                            ...
                            @if ($minute->status === 'published')
                                <div
                                    class="absolute top-0 right-0 px-3 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-bold uppercase rounded-bl-xl z-10">
                                    Đã duyệt
                                </div>
                            @else
                                <div
                                    class="absolute top-0 right-0 px-3 py-1 bg-orange-100 text-orange-700 text-[10px] font-bold uppercase rounded-bl-xl z-10">
                                    Chờ duyệt
                                </div>
                            @endif
                            ...
                            <div class="flex flex-col sm:flex-row gap-5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1 pr-16">
                                        <h3 class="text-lg font-bold text-slate-800 dark:text-white group-hover:text-primary transition-colors truncate"
                                            title="{{ $minute->title }}">
                                            {{ $minute->title }}
                                        </h3>
                                    </div>
                                    <div class="text-xs text-primary font-bold mb-2">
                                        {{ $minute->studentClass->code ?? 'Lớp ???' }} -
                                        {{ $minute->studentClass->name ?? '' }}
                                    </div>
                            ...
                            <div
                                class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                ...
                                <div class="flex gap-2 self-end sm:self-auto flex-wrap items-stretch">
                                    ...
                                    <a href="{{ route($routePrefix . 'minutes.export_pdf', $minute->id) }}"
